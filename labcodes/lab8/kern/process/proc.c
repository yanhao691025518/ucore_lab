...
alloc_proc(void) {
     ...
     */
     //LAB5 201201130 : (update LAB4 steps)
    /*
     * below fields(add in LAB5) in proc_struct need to be initialized	
     *       uint32_t wait_state;                        // waiting state
     *       struct proc_struct *cptr, *yptr, *optr;     // relations between processes
	 */
     //LAB6 2012011330 : (update LAB5 steps)
    /*
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    ret = -E_NO_MEM;
    //LAB4:EXERCISE2 2012011330
    //LAB8:EXERCISE2 2012011330  HINT:how to copy the fs in parent's proc_struct?
    /*
	...
	//LAB5 2012011330 : (update LAB4 steps)
   /* Some Functions
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2012011330  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
...
//    int i;
//    for (i = 1; i <= size; i++) {
//        cprintf("%02x ", *((unsigned char*)(binary) + i - 1));
//        if (i % 16 == 0)
//            cprintf("\n");
//    }
//    cprintf("\n");
    ...
    //assert((page = pgdir_alloc_page(mm->pgdir, USTACKTOP, PTE_USER)) != NULL);
    ...
    size_t push_size = 0;
    int i;
    void *stack_top = page2kva(page) + PGSIZE, *start, *start2;
    for (i = argc - 1; i >= 0; i--) {
        struct Page *argvi;
        start2 = USTACKTOP-USTACKSIZE+(argc-i)*PGSIZE;
        ...
        push_size += sizeof(void*);
        stack_top -= sizeof(void*);
    }
    memcpy(stack_top - sizeof(int), &argc, sizeof(int));
    push_size += sizeof(int);
