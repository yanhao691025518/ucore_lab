...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 2012011330 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	/*
	...
	*/
    ...
    if (blkoff != 0) {
        ...
        // sfs_bmap_load_nolock(sfs, sin, uint32_t index, uint32_t *ino_store);
...
//    int i;
//    cprintf("Buf:%x offset:%d\n", buf, offset);
//    for (i = 1; i <= 128; i++) {
//        cprintf("%02x ", *((unsigned char*)(old_buf) + i - 1));
//        if (i % 16 == 0)
//            cprintf("\n");
//    }
out:
